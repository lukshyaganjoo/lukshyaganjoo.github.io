...
In the setting where the number of marked inputs, i.e. $$\mathbf{x}^* \in \{0, 1\}^n$$ for which $$f(\mathbf{x}^*) = 1$$ is $$\mathcal{L}$$; we can follow a very similar procedure to the single marked input case. Define $$\mathcal{S} := \{\mathbf{x}^* \in \{0, 1\}^n : f(\mathb{x}^*) = 1\}$$ to be the set of marked inputs.
...
\begin{align*}
    ...
    \ket{B} &:= \frac{1}{\sqrt{N - \mathcal{L}}} \sum{x \notin \mathcal{S}} \ket{x}
\end{align*}
$$
</p>

The key difference here is that the initial angle $$\theta$$ is $$\arcsin(\sqrt{\frac{\mathcal{L}}{N}})$$. By symmetric argument to the single marked input case, the overall runtime to acheive constant success probability is $$\mathcal{O}(\sqrt{\frac{\mathcal{L}}{N}})$$.
