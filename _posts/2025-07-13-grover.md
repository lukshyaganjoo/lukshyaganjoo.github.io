...
$$\begin{quantikz}
\lstick{$|0\rangle^{\otimes n}$} & \gate{H^{\otimes n}} & \gate[2]{O_f} & \gate{R} & \cdots & \gate[2]{O_f} & \gate{R} & \gate{A} & \qw \\
& & & & & & & & \\
& & \gategroup[2,steps=4,style={dashed,rounded corners,fill=blue!20, inner xsep=2pt},background,label style={label position=below,anchor=north,yshift=-0.2cm}]{{$O(\sqrt{N})$ times}} & & & & & &
\end{quantikz}$$
