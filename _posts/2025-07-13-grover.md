...
Invoking the small-angle sine approximation, we know that $$\sin((2K + 1)\theta) \approx (2K + 1)\theta$$ and picking $$K = \mathcal{O}(\sqrt{N})$$ is sufficient to find the marked input with high probability. 
