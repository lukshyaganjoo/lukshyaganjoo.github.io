...
$$\begin{quantikz}
\lstick{$\ket{0}^{\otimes n}$} & \gate{\mathbf{H}^{\otimes n}} & \gate{\mathbf{O}_f} & \gate{\mathbf{R}} & \cdots & \gate{\mathbf{O}_f} & \gate{\mathbf{R}} & \meter{} & \qw \\
& & \gategroup[1,steps=4,style={dashed,rounded corners,fill=blue!20, inner xsep=2pt},background,label style={label position=below,anchor=north,yshift=-0.2cm}]{{$O(\sqrt{N})$ times}} & & & & & &
\end{quantikz}$$
