...
\end{align*}$$ 
which describes a reflection about $$\ket{+}^{\otimes n}$$ or a uniform superposition over all $$n$$-bit strings. for those of us who like thinking of transformations as matrices; $$\mathbf{R}$$ can be explicitly written as the matrix $$2 \ketbra{+}^{\otimes n} - \mathbb{I}$$. 
